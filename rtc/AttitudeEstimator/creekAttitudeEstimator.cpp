...
//
#include "creekAttitudeEstimator.h"
using namespace creek;

void calcRotFromRpy(Matrix3& out_R, double r, double p, double y)
{
...
static const char* module_spec[] =
  {
    "implementation_id", "creekAttitudeEstimator",
    "type_name",         "creekAttitudeEstimator",
...
creekAttitudeEstimator::creekAttitudeEstimator(RTC::Manager * manager)
...
RTC::ReturnCode_t creekAttitudeEstimator::onInitialize()
{
  std::cout << "creekAttitudeEstimator : onInitialize" << std::endl;
  ...
  std::cout << "creekAttitudeEstimator :";
  ...
  if ( ! coil::stringTo(m_dt, prop["dt"].c_str()) ) {
    std::cerr << "creekAttitudeEstimator failed to prop[dt] " << prop["dt"] << "" << std::endl;
...
RTC::ReturnCode_t creekAttitudeEstimator::onActivated(RTC::UniqueId ec_id)
{
  std::cout << "creekAttitudeEstimator : onActivated" << std::endl;
  return RTC::RTC_OK;
}


RTC::ReturnCode_t creekAttitudeEstimator::onDeactivated(RTC::UniqueId ec_id)
{
  std::cout << "creekAttitudeEstimator : onDeactivated" << std::endl;
  return RTC::RTC_OK;
}


RTC::ReturnCode_t creekAttitudeEstimator::onExecute(RTC::UniqueId ec_id)
{
  ...
  // gyro (on world)
  //
  Matrix3 rot; //( rotFromRpy(m_rpy.data[0], m_rpy.data[1], m_rpy.data[2]) );
  //calcRotFromRpy( rot, m_rpy.data[0], m_rpy.data[1], m_rpy.data[2] );
  calcRotFromRpy( rot, m_rpy.data.r, m_rpy.data.p, m_rpy.data.y );
  Vector3 tmp;  tmp << m_gyroSensor.data[0], m_gyroSensor.data[1], m_gyroSensor.data[2];
  Vector3 gyro( rot * tmp );
  ...
  std::cout << "creekAttitudeEstimator : " << rpyDeg(0) << ", " << rpyDeg(1) << ", " << rpyDeg(2) << std::endl;
  */  
...
extern "C"
{
  void creekAttitudeEstimatorInit(RTC::Manager* manager)
  {
    ...
    manager->registerFactory(profile,
                             RTC::Create<creekAttitudeEstimator>,
                             RTC::Delete<creekAttitudeEstimator>);
  }  
};
